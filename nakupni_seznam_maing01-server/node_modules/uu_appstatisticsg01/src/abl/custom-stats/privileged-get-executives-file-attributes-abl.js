"use strict";
// documentation: https://uuapp.plus4u.net/uu-bookkit-maing01/d11a43aa3a214083b84a4e4103a4f635/book/page?code=61900672
// template:
// templateVersion:
//@@viewOn:revision
...
class PrivilegedGetExecutivesFileAttributes {
  constructor() {
  ...
  async privilegedGetExecutivesFileAttributes(awid, dtoIn) {
    ...
    const uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      WARNINGS.privilegedGetExecutivesFileAttributes.code,
      Errors.PrivilegedGetExecutivesFileAttributes.InvalidDtoIn
    );

    // HDS 2
    const binaryStoreBaseUri = Config.get(Constants.binaryStoreBaseUri) || Config.get(Constants.defaultBinaryStore);
    if (!binaryStoreBaseUri) throw new Errors.PrivilegedGetExecutivesFileAttributes.MissingBinaryStore(uuAppErrorMap);

    // HDS 3
    const sysIdentitySession = await getSystemIdentity();
    //HDS 4
    const collection = [Constants.collections.uuAppStatistics, Constants.collections.executives];
...
module.exports = new PrivilegedGetExecutivesFileAttributes();
