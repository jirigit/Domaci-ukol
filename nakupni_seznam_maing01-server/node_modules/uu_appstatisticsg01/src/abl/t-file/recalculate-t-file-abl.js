// template: recalculate-t-file-abl
...
class RecalculateTFilesAbl {
  constructor() {
    this.validator = Validator.load();
  }

  async recalculateTFiles(dtoIn, authzContext) {
    //HDS 1
    let validationResult = this.validator.validate("uuAppstatisticsRecalculateTFileDtoInTypes", dtoIn);
    let uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      WARNINGS.recalculateTFilesUnsupportedKeys.code,
      Errors.RecalculateTFiles.InvalidDtoIn
    );

    //HDS 2
    const sysIdentitySession = await getSystemIdentity();
    const awid = sysIdentitySession.getClientIdentity().getAwid();
    ...
    if ((dtoIn.force || dtoIn.resetData) && !isAuthorities(authorizedProfiles)) {
      throw new Errors.RecalculateTFiles.UserIsNotAuthorized({ uuAppErrorMap }, { authorizedProfiles });
    }

    //HDS 3
    if (!Config.get(Constants.TFileStoreUri)) {
      throw new Errors.RecalculateTFiles.TFileStoreUriConfigIsNotSet({ uuAppErrorMap });
    }
    ...
    try {
      ...
      // HDS 3.1
      throw new Errors.RecalculateTFiles.RecalculateTFilesFailed({ uuAppErrorMap }, error);
    }
...
module.exports = new RecalculateTFilesAbl();
