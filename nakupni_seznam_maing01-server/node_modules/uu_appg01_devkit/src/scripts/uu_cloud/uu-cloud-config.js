...
class UuCloudConfig {
  constructor(projectRoot, projectName, projectVersion, targetDir, buildConfig, taskName) {
    ...
    if (devkitConfigFull) {
      this.devkitConfig = (((devkitConfigKey ? devkitConfigFull[devkitConfigKey] : devkitConfigFull) || {}).devkit ||
        {})[this.environment];
    } else {
  ...
  uuAppBoxConfig() {
    ...
    result.metaModelCode = `${appBoxNaming.code}/METAMODEL`;
    result.metaModelDir = this._getMetaModelDir();
  ...
  _getMetaModelDir() {
    if (!this.metaModelDir) {
      if (fs.existsSync((this.metaModelDir = path.resolve(this.projectRoot, "app/config/metamodel")))) {
      } else {
        this.metaModelDir = null;
      }
    }
    return this.metaModelDir;
  }
