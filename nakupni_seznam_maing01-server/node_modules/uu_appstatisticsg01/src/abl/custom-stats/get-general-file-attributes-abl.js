"use strict";
// documentation: https://uuapp.plus4u.net/uu-bookkit-maing01/d11a43aa3a214083b84a4e4103a4f635/book/page?code=11900457
// template:
// templateVersion:
//@@viewOn:revision
...
class GetGeneralFileAttributesAbl {
  constructor() {
  ...
  async getGeneralFileAttributes(awid, dtoIn) {
    ...
    const uuAppErrorMap = ValidationHelper.processValidationResult(
      dtoIn,
      validationResult,
      WARNINGS.getGeneralUnsupportedKeys.code,
      Errors.GetGeneralFileAttributes.InvalidDtoIn
    );

    // HDS 2
    const binaryStoreBaseUri = Config.get(Constants.binaryStoreBaseUri) || Config.get(Constants.defaultBinaryStore);
    if (!binaryStoreBaseUri) throw new Errors.GetGeneralFileAttributes.MissingBinaryStore(uuAppErrorMap);

    // HDS 3
    const sysIdentitySession = await getSystemIdentity();
    //HDS 4
    const collection = [Constants.collections.uuAppStatistics];
...
module.exports = new GetGeneralFileAttributesAbl();
