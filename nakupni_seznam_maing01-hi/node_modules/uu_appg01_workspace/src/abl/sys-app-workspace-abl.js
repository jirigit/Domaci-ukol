...
class SysAppWorkspaceAbl {
  ...
  async getUuAppMetamodel(uuIdentity, uuIdentityType) {
    const uuAppErrorMap = {};

    // HDS 1. - System checks that caller is systemIdentity
    if (!["uuAsidEe", "uuAwidEe"].includes(uuIdentityType)) {
      // A1
      throw new Errors.GetUuAppMetamodel.InvalidUuIdentityType({ uuAppErrorMap }, { uuIdentity, uuIdentityType });
    }

    // HDS 2. - System gets metamodel zip file from configuration
    const filename = "metamodel.zip";
