...
class uuAppBoxTask {
  ...
  async process() {
    ...
    let metaModelZip = await this._createMetaModelZip();
    ...
    await this._uploadMetaModel(appBoxUri, metaModelZip);
  }
  ...
  async _uploadMetaModel(appBoxUri, dataPath) {
    if (!this.config.libraryDescriptorPath && dataPath) {
      console.log(`Uploading meta model ${dataPath}.`);
      let code = this.config.metaModelCode;
      await this._uploadAttachment(appBoxUri, code, dataPath);
    }
  }

  async _createMetaModelZip() {
    if (!this.config.libraryDescriptorPath) {
      const metaModelDir = this.config.metaModelDir;
      if (!metaModelDir) {
        console.log("Warning: Meta model directory is missing");
        return null;
      }
      if (fs.readdirSync(metaModelDir).length == 0) {
        console.log("Warning: Meta model file is missing");
        return null;
      }
      const zip = require("../../tools/zip");
      let dataPath = path.resolve(metaModelDir, "..", "metamodel.zip");
      await zip(dataPath, (archive) => {
        archive.directory(metaModelDir, "metamodel");
      });
      return dataPath;
    } else {
      return null;
