...
class _Array extends Matcher {
  ...
  processParams(...params) {
    if (params.length > 0 && params.length <= 4) {
      ...
      if (params.length === 2) {
        ...
        this.maxSize = params[3];
        if (this.maxSize != null && (typeof this.maxSize !== "number" || this.maxSize < 0)) {
          throw new MatcherError("Array: invalid parameter3, expected Number greater than or equal to 0");
        }
      }
    } else if (params.length > 4) {
      throw new MatcherError("Array: wrong number of parameters, expected 0..4");
    }
  }
  ...
  match(value, ctx) {
    ...
    // Check via "Object.prototype.toString.call(value) !== '[object Array]'" must be used,
    // as "value.constructor !== Array" is not reliably working in sandbox (uuScriptEngine)
    if (Array.isArray(value) || Object.prototype.toString.call(value) === "[object Array]") {
      ...
      if (this.maxSize != null && Sizeof(value) > this.maxSize) {
        result.addInvalidValueError("array.e005", `The size of the array must not exceed ${this.maxSize} B.`);
      }
    } else {
