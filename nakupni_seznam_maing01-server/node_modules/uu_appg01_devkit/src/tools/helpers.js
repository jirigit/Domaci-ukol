    ...
    if (!("stdio" in spawnOpts)) {
      child.stderr.on("data", (chunk) => {
        ...
        for (let i = 0; i < lines.length; i++) {
          let line = lines[i];
          let textOnlyLine = line.replace(/\033\[[0-9;]+m/g, ""); // strip ANSI-escapes for terminal colors
          if (!textOnlyLine.match(/^\s*npm\s+ERR!\s+/)) {
            childFilteredStderr.write(line + "\n");
            if (copyToBuffer) process.stderr.write(line + "\n");
          } else {
            // if "npm ERR" was found then start bufferring so that we can omit it
            buffering = true;
            buffer = Buffer.concat([Buffer.from(lines.slice(i).join("\n") + "\n"), buffer]);
            break;
          }
        }
      });
